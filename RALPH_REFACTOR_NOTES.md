...
- Extracted structured progress parsing, bridge progress creation, timestamps, and event emission into `apps/client/src-tauri/src/desktop_task/progress.rs`.
- Extracted shared CLI stdout/stderr readers, worker joining with temp-file cleanup, child-process termination, hidden-window setup, and detached system-shell opening into `apps/client/src-tauri/src/desktop_task/process.rs`.
...
- Converted `apps/client/src-tauri/src/runtime_snapshot/model_catalog.rs` into a small facade that preserves `create_provider_model_http_client` and `fetch_provider_model_catalog` for the existing runtime snapshot call path.
...
- Extracted runtime snapshot collection, provider availability, active provider resolution, compatibility resolution, and agent-limit resolution from `apps/client/src-tauri/src/runtime_snapshot.rs` into `apps/client/src-tauri/src/runtime_snapshot/collect.rs`.
...
- Extracted runtime environment dotenv parsing and process-environment overrides from `apps/client/src-tauri/src/runtime_snapshot/env.rs` into `apps/client/src-tauri/src/runtime_snapshot/env_dotenv.rs`.
- Extracted PATH, PATHEXT, home-directory, default install-location, and executable file-name helpers into `apps/client/src-tauri/src/runtime_snapshot/env_paths.rs`.
- Extracted configured binary validation, command-on-PATH resolution, packaged Windows app alias filtering, and agent CLI binary resolution into `apps/client/src-tauri/src/runtime_snapshot/env_commands.rs`.
...
- Extracted runtime snapshot user settings load/save helpers, user API-key persistence, user web-search settings, voice and speech-to-text settings, memory settings, review model settings, desktop settings, and environment merge helpers into `apps/client/src-tauri/src/runtime_snapshot/settings_commands.rs`.
...
- Extracted Ralph active-run progress state, block progress snapshots, metadata readers, event tone/progress labels, timestamp formatting, and block-detail sorting from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-active-run-progress.helper.ts`.
...
- Added focused helper coverage in `apps/client/src/tauri/ui/ralph/_helpers/ralph-active-run-progress.helper.spec.ts` for timeline metadata snapshots, active-run event updates, streamed block progress, and block detail ordering.
- `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over the 500-line policy after this bounded pass because it still owns the flow library, canvas editing, inspector, generation, run setup, live run, history/detail/log panels, and dialog composition. Further splits should target one of those UI responsibilities at a time.

## src pass 2

- Extracted Ralph inspector width bounds, local-storage load/save behavior, and scroll epsilon constants from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-inspector-width.helper.ts`.
- Added focused coverage in `apps/client/src/tauri/ui/ralph/_helpers/ralph-inspector-width.helper.spec.ts` for width clamping, viewport caps, invalid stored values, valid stored values, and preference writes.
...
- Extracted Ralph flow editor block creation, copied-block creation, edge ID generation, and default utility config construction from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-block-factory.helper.ts`.
...
- Extracted Ralph flow row run labels, flow status presentation, run status presentation, and block output chip class mapping from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-run-presentation.helper.ts`.
...
- Extracted Ralph generation activity event normalization, result-event normalization, deduplicated activity appending, and job activity application from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-generation-activity.helper.ts`.
...
- Extracted Ralph starter-flow summary construction, starter lookup, imported-starter update detection, import ID creation, subtitle formatting, and starter icon mapping from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-starter-flow-presentation.helper.ts`.
...
- Extracted Ralph run record block cards, active run block detail cards, streamed progress rows, and output-section details from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-run-detail-cards.tsx`.
...
- Verified the extracted render path with `pnpm typecheck:ui` and `pnpm test:ui apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`.
- `apps/client/src/tauri/ui/ralph/components/ralph-run-detail-cards.tsx` is below the 500-line policy at 316 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy because it still owns editor state wiring, canvas interactions, inspector forms, generation dialogs, and run panel composition.

## src pass 9

- Extracted the starter-flow import dialog and expanded field editor dialog from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-editor-dialogs.tsx`.
...
- Verified the extracted render path with `pnpm typecheck:ui` and `pnpm test:ui apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`.
- `apps/client/src/tauri/ui/ralph/components/ralph-editor-dialogs.tsx` is below the 500-line policy at 307 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy because it still owns editor state wiring, canvas interactions, inspector forms, generation interview composition, and run panel composition.

## src pass 10

- Extracted shared Ralph inspector field and details primitives from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-inspector-primitives.tsx`.
...
- Verified the extracted render path with targeted ESLint, `pnpm typecheck:ui`, and `pnpm test:ui apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`.
- `apps/client/src/tauri/ui/ralph/components/ralph-inspector-primitives.tsx` is below the 500-line policy. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy because it still owns editor state wiring, canvas interactions, inspector forms, generation interview composition, and run panel composition.

## src pass 11

- Extracted Ralph flow editor mode types, option lists, provider option construction, UI limits, context-menu sizing, and React Flow pro options from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-flow-editor-options.helper.ts`.
...
- `apps/client/src/tauri/ui/ralph/_helpers/ralph-flow-editor-options.helper.ts` is below the 500-line policy. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy because it still owns editor state wiring, canvas interactions, inspector forms, generation interview composition, and run panel composition.

## src pass 12

- Extracted the Ralph generation interview dialog from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-generation-interview-dialog.tsx`.
...
- Verified the extracted render path with `pnpm typecheck:ui`, `pnpm lint`, and `pnpm test:ui apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`.
- `apps/client/src/tauri/ui/ralph/components/ralph-generation-interview-dialog.tsx` is below the 500-line policy at 355 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 13

- Extracted Ralph flow editor state utilities, input-field ID creation, context-menu placement, save/revision message formatting, prompt-like block text updates, utility-title sync detection, history limits, flow snapshots, layout keys, canvas position comparisons, and locked-node change filtering from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/_helpers/ralph-flow-editor-state.helper.ts`.
...
- Added focused helper coverage in `apps/client/src/tauri/ui/ralph/_helpers/ralph-flow-editor-state.helper.spec.ts` for field IDs/defaults, save messages, prompt-like updates, utility title syncing, flow snapshots/layout keys, position/size comparisons, locked position changes, non-browser menu placement, and missing-DOM shortcut targets.
- `apps/client/src/tauri/ui/ralph/_helpers/ralph-flow-editor-state.helper.ts` is below the 500-line policy at 197 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy at 11542 lines because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 14

- Extracted the main Ralph flow editor canvas toolbar from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-flow-editor-toolbar.tsx`.
...
- `apps/client/src/tauri/ui/ralph/components/ralph-flow-editor-toolbar.tsx` is below the 500-line policy. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 15

- Extracted Ralph flow-list and canvas context menus from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-flow-context-menus.tsx` and `apps/client/src/tauri/ui/ralph/components/ralph-node-context-menu-content.tsx`.
...
- Extracted Ralph prompt placeholder highlighting from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-prompt-highlight.tsx`.
...
- Added focused component coverage in `apps/client/src/tauri/ui/ralph/components/ralph-prompt-highlight.spec.tsx` for plain prompt text and multiple Ralph placeholders.
- Verified with `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/components/ralph-prompt-highlight.spec.tsx`, `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`, `pnpm typecheck`, `pnpm lint`, and `pnpm test`.
- `apps/client/src/tauri/ui/ralph/components/ralph-prompt-highlight.tsx` is below the 500-line policy at 39 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy at 11683 lines because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.
- Both new context-menu component files are below the 500-line policy. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 17

- Extracted Ralph inspector section tabs and selected route summary rendering from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-inspector-navigation.tsx`.
...
- Added focused component coverage in `apps/client/src/tauri/ui/ralph/components/ralph-inspector-navigation.spec.tsx` for hidden single-section tabs, route badge selection callbacks, route summary labels, and route summary open callbacks.
- Verified with `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/components/ralph-inspector-navigation.spec.tsx`, `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`, `pnpm typecheck`, `pnpm lint`, and `pnpm test`.
- `apps/client/src/tauri/ui/ralph/components/ralph-inspector-navigation.tsx` is below the 500-line policy at 144 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy at 11597 lines because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 18

- Extracted Ralph pending input controls and setup variable controls from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-input-controls.tsx`.
...
- Added focused component coverage in `apps/client/src/tauri/ui/ralph/components/ralph-input-controls.spec.tsx` for file, number, boolean variable, and error attribute behavior.
- Verified with `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/components/ralph-input-controls.spec.tsx`, `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`, `pnpm typecheck`, `pnpm typecheck:ui`, `pnpm lint`, and `pnpm test`.
- `apps/client/src/tauri/ui/ralph/components/ralph-input-controls.tsx` is below the 500-line policy at 240 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy at 11397 lines because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 19

- Extracted the Ralph flow library sidebar from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-flow-library-panel.tsx`.
...
- Added focused component coverage in `apps/client/src/tauri/ui/ralph/components/ralph-flow-library-panel.spec.tsx` for active run status labels, starter update labels, draft status labels, empty workspace disabled actions, library mode callbacks, and collapsed open behavior.
- Verified with `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/components/ralph-flow-library-panel.spec.tsx` and `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx` before broad checks.
- `apps/client/src/tauri/ui/ralph/components/ralph-flow-library-panel.tsx` is below the 500-line policy at 448 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy at 11174 lines because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 20

- Extracted Ralph AI prompt history ArrowUp/ArrowDown navigation state into `apps/client/src/tauri/ui/ralph/_helpers/normalize-ralph-ai-prompt-history.helper.ts`.
...
- Verified with `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/_helpers/normalize-ralph-ai-prompt-history.helper.spec.ts`, `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`, `pnpm typecheck:ui`, and targeted ESLint for the touched Ralph editor/history files.
- `apps/client/src/tauri/ui/ralph/_helpers/normalize-ralph-ai-prompt-history.helper.ts` is below the 500-line policy at 83 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy at 10494 lines because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.

## src pass 21

- Extracted Ralph utility condition form rendering from `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` into `apps/client/src/tauri/ui/ralph/components/ralph-utility-condition-fields.tsx`.
...
- Added focused component coverage in `apps/client/src/tauri/ui/ralph/components/ralph-utility-condition-fields.spec.tsx` for default simple rendering, expression updates, and JSON-path operator updates.
- Verified with `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/components/ralph-utility-condition-fields.spec.tsx`, `pnpm exec vitest run --config vitest.ui.config.ts apps/client/src/tauri/ui/ralph/ralph-flow-editor.spec.tsx`, `pnpm typecheck:ui`, and targeted ESLint for the touched Ralph files.
- `apps/client/src/tauri/ui/ralph/components/ralph-utility-condition-fields.tsx` is below the 500-line policy at 158 lines. `apps/client/src/tauri/ui/ralph/ralph-flow-editor.tsx` remains over policy at 11045 lines because it still owns editor state wiring, canvas interactions, inspector forms, generation setup, and run panel composition.
