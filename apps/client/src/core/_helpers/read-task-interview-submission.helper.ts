...
export const TASK_INTERVIEW_SECTION_TITLE = "Task interview round";

export const TASK_INTERVIEW_INPUT_TYPES = [
  "text",
  "textarea",
  "number",
  "boolean",
...
export interface TaskInterviewSubmission {
  complete: boolean;
  summary?: string;
...
const isTaskInterviewInputType = (
  value: string,
): value is RalphInputFieldType => {
  return TASK_INTERVIEW_INPUT_TYPES.includes(value as RalphInputFieldType);
};

const normalizeTaskInterviewFieldId = (
  value: unknown,
  fallback: string,
): string => {
  const normalized =
...
const normalizeTaskInterviewFieldType = (
  value: unknown,
): RalphInputFieldType => {
  ...
  return isTaskInterviewInputType(normalized) ? normalized : "textarea";
};

const normalizeTaskInterviewOptions = (
  value: unknown,
...
const normalizeTaskInterviewValidation = (
  value: unknown,
...
const normalizeTaskInterviewHelp = (value: unknown): string | undefined => {
  if (typeof value !== "string") {
    return undefined;
  }
...
const normalizeTaskInterviewInputValue = (
  value: unknown,
...
const normalizeTaskInterviewStringArray = (value: unknown): string[] => {
...
export const normalizeTaskInterviewSubmission = (
  value: unknown,
): TaskInterviewSubmission => {
  if (!isRecord(value)) {
  ...
  const fields = questions
    .flatMap((entry, index): RalphInputField[] => {
      ...
      const type = normalizeTaskInterviewFieldType(entry.type);
      const options = normalizeTaskInterviewOptions(entry.options);
      ...
      const defaultValueSource =
        "defaultValue" in entry && entry.defaultValue !== null
          ? entry.defaultValue
          : entry.default;
      const defaultValue = normalizeTaskInterviewInputValue(defaultValueSource);
      const validation = normalizeTaskInterviewValidation(entry.validation);
      const help = normalizeTaskInterviewHelp(entry.help);

      return [
        {
          id: normalizeTaskInterviewFieldId(entry.id, `question_${index + 1}`),
          label,
          type,
  ...
  return {
    ...
    ...(typeof value.contextSummary === "string" && value.contextSummary.trim()
      ...
      : {}),
    findings: normalizeTaskInterviewStringArray(value.findings),
    assumptions: normalizeTaskInterviewStringArray(value.assumptions),
    relevantFiles: normalizeTaskInterviewStringArray(value.relevantFiles),
    fields,
  };
};

const parseTaskInterviewJsonCandidate = (text: string): unknown => {
  const trimmed = text.trim();
  const taggedMatch = trimmed.match(
    /<machdoch_task_interview>\s*([\s\S]*?)\s*<\/machdoch_task_interview>/iu,
  );
...
export const readTaskInterviewSubmission = (
  result: TaskExecutionResult,
): TaskInterviewSubmission => {
  const candidates = [
    ...result.outputSections
      .filter((section) => section.title === TASK_INTERVIEW_SECTION_TITLE)
  ...
  for (const candidate of candidates) {
    try {
      return normalizeTaskInterviewSubmission(
        parseTaskInterviewJsonCandidate(candidate),
      );
    } catch {
