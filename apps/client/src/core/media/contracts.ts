...
export type MediaFlowGroupColor =
  | "slate"
  | "cyan"
  | "violet"
  | "amber"
...
export interface MediaDiscoveredModelArtifact {
  ...
  relativePath: string;
  displayName: string;
  kind: "diffusers-model" | "checkpoint" | "model-addon" | "source-repository";
  status:
    | "ready"
    | "importable"
    | "incompatible"
...
export interface MediaCivitaiSampleImage {
  url: string;
  width: number | null;
  height: number | null;
}
...
export interface MediaImageMaskPoint {
  x: number;
  y: number;
}
...
export interface MediaGenerationAssetMetadata {
  categoryIds: string[];
  tags: string[];
  triggerWords: string;
  sourceUrl: string | null;
...
export interface MediaLocalVideoGenerationOperation extends Omit<
  MediaLocalWanVideoGenerationOperation,
  "kind" | "providerId" | "modelId" | "conv3dBackend" | "conditioningMode"
> {
...
export type MediaAssetExportMode = "verified-original" | "metadata-stripped";
...
export interface EnqueueMockRemoteRunRequest extends EnqueueFixtureRunRequest {
