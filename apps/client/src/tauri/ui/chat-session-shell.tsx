...
import { useSchedulerActivity } from "./scheduler/use-scheduler-activity";
import {
...
export const ChatSession = (): JSX.Element => {
  ...
  const schedulerActivity = useSchedulerActivity(
  ...
  useEffect(() => {
    const activeOperationIds = new Set(controller.activeChatOperationIds);
    const completedOperationIds = getCompletedOperationIds(
      previousChatOperationIdsRef.current,
      activeOperationIds,
    );

    if (completedOperationIds.length > 0 && activeApp !== "chat") {
      setChatCompletedSinceView(true);
    }

    previousChatOperationIdsRef.current = activeOperationIds;
  }, [activeApp, controller.activeChatOperationIds]);
