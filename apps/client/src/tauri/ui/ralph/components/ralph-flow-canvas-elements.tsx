...
const RALPH_SELECTED_RESIZER_LINE_CLASS_NAME = "!border-sky-100/90";
const RALPH_SELECTED_RESIZER_HANDLE_CLASS_NAME =
  "!h-2.5 !w-2.5 !border-sky-100 !bg-slate-950 !shadow-[0_0_12px_rgba(56,189,248,0.75)]";
...
const RalphNoteNode = ({
  ...
  return (
    <div
      className={cn(
        "relative flex h-full min-h-[120px] w-full min-w-[180px] flex-col overflow-hidden rounded-2xl border shadow-lg shadow-black/20",
    ...
    >
      ...
      />
      <div
        className={cn("h-1 shrink-0", getAnnotationAccentClassName(block.tone))}
      />
...
const RalphBlockNode = ({
  ...
  const isSelected = selected || data.selected;
  const locked = (data.block.locked ?? false) || Boolean(data.lockedByGroupId);
