...
describe("normalizeShellState", () => {
  it("persists only typed terminal outcomes", () => {
      ...
      messages: [
        {
          ...
          role: "agent",
          content: "Timed out",
          outcome: { status: "timed-out", reason: "Absolute limit" },
        },
        {
          id: "invalid",
          taskId: "task-2",
          role: "agent",
          content: "Quoted success",
          outcome: { status: "completed-successfully" },
        } as never,
      ],
    });
    ...
    expect(normalized.sessions[0]?.messages[0]?.outcome).toEqual({
      status: "timed-out",
      reason: "Absolute limit",
    });
    expect(normalized.sessions[0]?.messages[1]?.outcome).toBeUndefined();
  });
...
describe("applySessionRetentionPolicy", () => {
  ...
  it("retains an expired session while it owns helper work", () => {
    const now = Date.now();
    const session = createSession({
      id: "owned-helper-session",
      archivedAt: now - 8 * SESSION_DAY_MS,
      updatedAt: now - 8 * SESSION_DAY_MS,
      messages: [
        {
...
describe("getLatestRunningTaskId", () => {
  it("tracks chat runs and helper operations independently", () => {
    const session = createSession({
      messages: [
        {
          id: "chat-user",
    ...
    expect(getActiveChatOperationIds(session)).toEqual([
      "chat-run-1",
...
describe("recoverInterruptedTasksForLaunch", () => {
  it("marks persisted in-progress task groups as crashed once per app launch", () => {
    ...
    expect(crashMessages.map((message) => message.outcome?.status)).toEqual([
      "crashed",
      "crashed",
    ]);
  ...
  it.each([
    ...
    ["crashed", "crashed"],
  ] as const)("reports %s outcomes as %s", (outcome, expectedStatus) => {
    const session = createSession({
      messages: [
        {
          id: "user",
    ...
    expect(getSessionOverviewStatus(session)).toBe(expectedStatus);
  });

  it("derives unsupported execution outcomes without reporting success", () => {
    const execution = {
      ...createMockExecutionFixture("Unsupported operation"),
    ...
    expect(getSessionOverviewStatus(session)).toBe("unsupported");
  });
});
