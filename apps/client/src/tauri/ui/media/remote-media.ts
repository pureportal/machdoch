...
import type { RemoteControlCommandEvent } from "../runtime";
...
let runtimeInitialization: ReturnType<typeof initializeMediaRuntime> | null =
  null;
let remoteMediaOperationError: string | null = null;

export async function loadRemoteMediaSnapshot(
  configuredProviderIds: readonly string[],
): Promise<ProductMedia> {
  const runtime = await getRemoteMediaRuntime();
  ...
  return {
    loading: false,
    ...(remoteMediaOperationError ? { error: remoteMediaOperationError } : {}),
...
export function unavailableRemoteMediaSnapshot(
  reason: unknown,
...
export async function executeRemoteMediaCommand(
  ...
  if (command.kind === "cancel-media-run") {
    ...
    remoteMediaOperationError = null;
    return;
  }
...
function readGenerationCommand(command: RemoteControlCommandEvent): {
  prompt: string;
  target: "image" | "svg";
  modelId: string;
...
function getRemoteMediaRuntime(): ReturnType<typeof initializeMediaRuntime> {
