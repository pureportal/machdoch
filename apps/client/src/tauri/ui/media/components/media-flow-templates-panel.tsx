    ...
    try {
      ...
      setNotice(
        error instanceof Error
          ? error.message
          : "The template could not be forked.",
      );
    }
  };
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          aria-label="Close flow templates"
          onClick={onClose}
        >
          <X className="h-4 w-4" />
        </Button>
              ...
              <dl className="mt-3 grid grid-cols-2 gap-1.5 text-[9px]">
                ...
                <div className="rounded border border-slate-800 bg-slate-950/30 p-2">
                  ...
                  >
                    {errors.length === 0
                      ? "Ready"
                      : `${errors.length} setup item${errors.length === 1 ? "" : "s"}`}
                  </dd>
                </div>
              </dl>
                ...
                <div className="space-y-3 border-t border-slate-800 p-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {template.flow.nodes.map((node) => (
                      <span
                        key={node.id}
                        className="rounded border border-slate-800 px-1.5 py-0.5 text-[8px] text-slate-600"
                      >
                        {node.label}
                      </span>
                    ))}
                  ...
                  <dl className="space-y-1 text-[9px] text-slate-500">
                    <div className="flex justify-between gap-3">
                      <dt>VRAM estimate</dt>
                      <dd>
                        {plan.preflight.estimatedVramGb === null
                          ? "Unresolved"
                          : `${plan.preflight.estimatedVramGb} GB`}
                      </dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt>Model download</dt>
                      <dd>
                        {plan.preflight.estimatedDownloadGb === null
                          ? "None"
                          : `${plan.preflight.estimatedDownloadGb} GB`}
                      </dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt>Human review</dt>
                  ...
                  {errors.length > 0 ? (
                    <ul className="space-y-1 text-[9px] leading-4 text-amber-200/70">
                      {errors.slice(0, 3).map((diagnostic) => (
                        <li
                          key={`${diagnostic.code}-${diagnostic.nodeId ?? "flow"}`}
                        >
                          {diagnostic.message}
                        </li>
                      ))}
                ...
                >
                  ...
                  <div className="mt-2 flex gap-2">
                    ...
                    <Button
                      ...
                      className="h-7 px-2 text-[9px] text-slate-400"
                    >
                      Cancel
                    </Button>
                  </div>
