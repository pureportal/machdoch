...
const getLatestQuestionScope = (state: ChatInterviewDialogState): string => {
  return (
...
const getInterviewStatusTitle = (state: ChatInterviewDialogState): string => {
...
const getPrimaryActionLabel = (state: ChatInterviewDialogState): string => {
          ...
          <div className="grid min-h-0 grid-rows-[minmax(0,1fr)_auto] bg-slate-950">
            <ScrollArea className="min-h-0 bg-slate-950" type="always">
              <div className="grid gap-4 p-5">
                ...
                {["blocked", "cancelled", "timed-out", "failed"].includes(
                  state.status,
                ) ? (
