...
describe("provider sync coordinator", () => {
  it("keeps persistent provider sync opt-in by default", () => {
    expect(DEFAULT_PROVIDER_ENROLLMENT_CONFIG.persistentSync.enabled).toBe(false);
  });
  ...
  it("removes provider instruction projections when canonical instructions are deleted", async () => {
    ...
    await Promise.all([
      ...
      writeFile(join(userConfigRoot, "instructions.md"), "User policy.\n", "utf8"),
      writeFile(
        join(workspaceRoot, ".machdoch", "instructions.md"),
        "Workspace policy.\n",
        "utf8",
  ...
  it("removes stale projections when a provider is disabled in configuration", async () => {
    ...
    vi.stubEnv("MACHDOCH_USER_CONFIG_DIR", userConfigRoot);
    vi.stubEnv("CODEX_HOME", codexHome);
    const createConfig = (enabled: boolean): string => `${JSON.stringify({
      agentCliPaths: { "codex-cli": process.execPath },
      providerEnrollment: {
        enabled: true,
        persistentSync: { enabled: true, watch: false, daemonAtLogin: false },
    ...
    await Promise.all([
      writeFile(join(userConfigRoot, "user-config.json"), createConfig(true), "utf8"),
      writeFile(join(userConfigRoot, "instructions.md"), "User policy.\n", "utf8"),
    ]);
    await reconcileProviderSync(workspaceRoot);
    await writeFile(join(userConfigRoot, "user-config.json"), createConfig(false), "utf8");
  ...
  it("backs up and removes provider-native instructions and MCP entries before enablement", async () => {
    ...
    await Promise.all([
      ...
      mkdir(join(workspaceRoot, ".github", "instructions"), { recursive: true }),
    ...
    await Promise.all(files.map((path) => writeFile(path, "Provider policy.\n", "utf8")));
    await writeFile(
      join(workspaceRoot, ".machdoch", "instructions.md"),
    ...
    await Promise.all([
      ...
      writeFile(join(workspaceRoot, "docs", "README.md"), "Keep documentation.\n", "utf8"),
      writeFile(
    ...
    await Promise.all(
      files.map((path) => expect(stat(path)).rejects.toMatchObject({ code: "ENOENT" })),
    );
    await expect(
      readFile(join(workspaceRoot, ".machdoch", "instructions.md"), "utf8"),
    ).resolves.toContain("Canonical Machdoch policy");
    await expect(readFile(join(workspaceRoot, "docs", "README.md"), "utf8"))
      .resolves.toContain("Keep documentation");
    await expect(readFile(join(codexHome, "config.toml"), "utf8"))
      .resolves.toBe('model = "gpt-5.5"\nproject_doc_fallback_filenames = ["README.md"]\n');
    await expect(readFile(join(workspaceRoot, ".codex", "config.toml"), "utf8"))
      .resolves.toBe('model_verbosity = "low"\n');
    await expect(readFile(join(userHome, ".claude.json"), "utf8"))
      .resolves.toContain('"theme": "dark"');
    await expect(readFile(join(copilotHome, "mcp-config.json"), "utf8"))
      .resolves.toContain('"note": "keep"');
    await expect(stat(join(workspaceRoot, ".mcp.json"))).rejects.toMatchObject({ code: "ENOENT" });
  });
});
