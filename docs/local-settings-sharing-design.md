...
Use cancellation-aware `tokio::select!` operations so transfer shutdown reacts directly.
...
| `mcp.global` / **MCP Servers**                     | User config root `mcp.json`                                                                          | Workspace MCP config, discovery caches, lifecycle/usage state, generated provider files                                                                                                       | On, with credential/path warning      |
...
`persistentSync.{enabled,watch,debounceMs,fullRescanIntervalMs}`.
Preserve `persistentSync.daemonAtLogin` locally. Persistent sync projects MCP
only; it must never write the instruction library or provider instruction
files.
...
- prompt suffix, frontmatter parser, regular-file status, and 128 KiB bound;
...
If any replace or verification step fails, roll back every selected category from backups before releasing locks. On application startup, recovery must run before provider enrollment, MCP lifecycle, settings UI, scheduler, Fleet Manager, or other consumers start:
...
| MCP Servers     | Replace the whole global canonical `mcp.json` value/presence. Stop/reload affected MCP servers only after commit. Preserve workspace MCP and caches.                                           |
...
| Fleet Manager                   | `fleet-connection.json`, `fleet-control.json`, enrollment credentials, pending commands             | Would copy enrollment or control state                                     |
...
Version negotiation requires the exact current schema for each category. A
receiver preserves categories with any other schema as incompatible; it never
converts, silently drops unknown fields, or accepts a schema with “best
effort.”
...
| Instructions                       | User config root `instruction-library.json`: central files and their settings, workspace tags, and manual assignments                                                                                           | Provider-native files are outside Machdoch's instruction model                                       | Exclude from Settings transfer. Use the instruction CLI import/export workflow; optional workspace mappings are root-free and require explicit relinking.               |
...
| MCP                                | User config root `mcp.json`                                                                                                                               | `<workspace>/.machdoch/mcp/mcp.json`                                                                 | Transfer global canonical config only. Exclude both global/workspace caches and lifecycle state.                                             |
...
| Appearance and UI preferences      | Tauri store `machdoch-shell-state.json` plus the revisioned shell-state snapshot, under keys including appearance, RALPH settings, running-task behavior, and shell state                    | Many values inside shell/session state carry workspace roots or histories                            | Extract only closed category projections. Never serialize either mixed store or shell snapshot wholesale.                                                               |
...
| Fleet Manager                      | User config root `fleet-connection.json` and `fleet-control.json`                                                                                                                                               | Commands and product snapshots can reference workspace state                                         | Always exclude.                                                                                                                                                         |
...
| Shell-state CAS and cross-window operation lease                        | Detect concurrent UI/store changes                             | Never copy shell snapshots. Add category-key export/import and transaction recovery around only portable preference keys.                                                      |
...
| Fleet connection and control state                                      | Managed-state and UI status patterns                           | It contains enrollment credentials and remote-management command state. Do not reuse it as the settings-transfer transport.                                                    |
...
- Add pure validators for each category. Expose RALPH flow JSON validation without writing a live flow or requiring a workspace-capable command.
...
- Instruction libraries remain outside Settings transfer and keep their own
  import/export workflow.
- File-tree adapter: prompts with strict relative paths, metadata/link checks,
  and deterministic enumeration.
- MCP adapter: global `mcp.json`, stronger schema validation, existing CAS/permissions, lifecycle pause/reload.
...
- Store adapter: portable preference keys only, CAS fingerprints, no session snapshot access.
...
- Verify MCP global config replaces while workspace config, caches, and lifecycle state remain unchanged.
- Verify the instruction library remains byte-for-byte unchanged and prompt
...
- scheduler, shell sessions, attachments, Fleet product snapshot, Media Studio, and provider-enrollment workspace registries;
...
- Test sender-newer, receiver-newer, unknown category, unsupported schema, empty intersection, and malformed source category.
...
5. **MCP category composition:** Decide whether an absent sender `mcp.json` removes the receiver file or writes the canonical empty/default document; both are logically empty but have different byte/presence semantics.
