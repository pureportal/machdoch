...
- TXT: only `txtvers=1`, `protovers=1`, and a random 128-bit single-use `sid`. Keep it well below 200 bytes.
...
The shell snapshot also carries voice-reply values such as `autoSpeakResponses`, `rate`, and `preferredVoiceURI`. Preserve all of them in V1: the first two are portable but currently mixed into session shell state, while the URI is OS/device-specific. Move the portable pair to a dedicated global preference key before adding them to a future category. Likewise preserve the running-task action, onboarding state, app navigation state, RALPH UI workspace root/model prompt history, and every other shell-store key.
...
| Newer protocol minor | Use only explicitly negotiated messages/features; never best-effort parse unknown required fields | Unchanged on negotiation failure | Normal cleanup |
...
| Appearance and UI preferences | Tauri store `machdoch-shell-state.json` under keys including `machdoch.desktop.appearance-state`, RALPH settings, marketplace state, and shell state | Many values inside shell/session state carry workspace roots | Extract only the appearance and MCP registry keys. Never serialize the store file or RALPH UI settings wholesale. |
