...
export const loadMediaStudioState = async (): Promise<MediaStudioState> => {
  ...
  const stored = isRemoteMedia()
    ? normalizeMediaStudioState(await invoke("media_read_studio_state"))
    : await loadStoredValue<MediaStudioState>({
...
export const saveMediaStudioState = (
  ...
  const save = async (): Promise<void> => {
    if (isRemoteMedia()) {
      await invoke("media_write_studio_state", { value });
      return;
    }
