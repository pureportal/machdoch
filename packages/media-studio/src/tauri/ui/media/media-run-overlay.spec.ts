...
describe("projectMediaRunOverlay", () => {
  it("follows durable node state through success and clears active nodes at completion", () => {
    ...
    expect(
      projectMediaRunOverlay({ flow, plan, run: queued })?.activeNodeIds,
    ).toEqual([]);
    expect(
      projectMediaRunOverlay({ flow, plan, run: running })?.activeNodeIds,
    ).toEqual(["task"]);
    expect(
      projectMediaRunOverlay({ flow, plan, run: completed })?.activeNodeIds,
    ).toEqual([]);
  });
  ...
  it("does not mark a queued retry as the active node", () => {
    ...
    expect(
      projectMediaRunOverlay({ flow, plan, run: retrying })?.activeNodeIds,
    ).toEqual([]);
  });
