...
const validateVariableValue = (
  ...
  switch (variable.type) {
    ...
    case "number": {
      ...
      const steps =
        (value - variable.constraints.min) / variable.constraints.step;
    ...
    case "choice":
      return typeof value === "string" &&
        variable.constraints.options.includes(value)
        ? null
...
export const validateMediaFlowVariableDocument = (
  ...
  if (flow.variables.length > MAX_VARIABLES) {
    issues.push(
      schemaIssue(`Media flows support at most ${MAX_VARIABLES} variables.`),
    );
  }
  if (flow.presets.length > MAX_PRESETS) {
    issues.push(
      schemaIssue(`Media flows support at most ${MAX_PRESETS} presets.`),
    );
  }
  ...
  for (const variable of flow.variables) {
    if (!/^[a-z][a-z0-9_-]{0,63}$/u.test(variable.id)) {
      issues.push(
        schemaIssue(
          "Variable ids must be lowercase, bounded, and token-safe.",
          variable.id,
        ),
      );
    }
    if (variablesById.has(variable.id)) {
      issues.push(
        schemaIssue(`Variable id ${variable.id} is duplicated.`, variable.id),
      );
    }
    variablesById.set(variable.id, variable);
    if (
      !isTrimmedBoundedText(variable.name, 80) ||
      variable.name.length === 0
    ) {
      issues.push(
        schemaIssue(
          `Variable ${variable.id} requires a bounded display name.`,
          variable.id,
        ),
      );
    }
    if (!isTrimmedBoundedText(variable.description, 500, true)) {
      issues.push(
        schemaIssue(
          `Variable ${variable.id} has an invalid description.`,
          variable.id,
        ),
      );
    }
    ...
    switch (variable.type) {
      case "text":
        if (
          ...
          issues.push(
            schemaIssue(
              `${variable.name} has an invalid text length limit.`,
              variable.id,
            ),
          );
        }
      ...
      case "number":
        if (
          ...
          issues.push(
            schemaIssue(
              `${variable.name} has invalid numeric constraints.`,
              variable.id,
            ),
          );
        }
      ...
      case "boolean":
        if (Object.keys(variable.constraints).length > 0) {
          issues.push(
            schemaIssue(
              `${variable.name} has unsupported boolean constraints.`,
              variable.id,
            ),
          );
        }
      ...
      case "choice": {
        ...
        if (
          ...
          issues.push(
            schemaIssue(
              `${variable.name} requires 1–64 unique bounded options.`,
              variable.id,
            ),
          );
        }
    ...
    if (variable.defaultValue !== null) {
      ...
      if (reason)
        issues.push(schemaIssue(`Invalid default: ${reason}`, variable.id));
    }
  }
  ...
  for (const [variableId, value] of Object.entries(flow.variableBindings)) {
    ...
    if (!variable) {
      issues.push(
        schemaIssue(
          `Binding ${variableId} does not reference a declared variable.`,
          variableId,
        ),
      );
      continue;
  ...
  for (const preset of flow.presets) {
    if (
      ...
      issues.push(
        schemaIssue(`Preset id ${preset.id} is invalid or duplicated.`),
      );
    }
    presetIds.add(preset.id);
    if (!isTrimmedBoundedText(preset.name, 80) || preset.name.length === 0) {
      issues.push(
        schemaIssue(`Preset ${preset.id} requires a bounded display name.`),
      );
    }
    if (!isTrimmedBoundedText(preset.description, 500, true)) {
      issues.push(
        schemaIssue(`Preset ${preset.id} has an invalid description.`),
      );
    }
    for (const [variableId, value] of Object.entries(preset.values)) {
      ...
      if (!variable) {
        issues.push(
          schemaIssue(
            `Preset ${preset.name} references unknown variable ${variableId}.`,
          ),
        );
        continue;
      }
      ...
      if (reason)
        issues.push(
          schemaIssue(`Preset ${preset.name}: ${reason}`, variableId),
        );
    }
  }
  if (flow.activePresetId !== null && !presetIds.has(flow.activePresetId)) {
    issues.push(
      schemaIssue(`Active preset ${flow.activePresetId} does not exist.`),
    );
  }
  return issues;
};
...
export const getMediaFlowVariableValue = (
  ...
  if (Object.hasOwn(flow.variableBindings, variable.id)) {
    return {
      value: flow.variableBindings[variable.id] ?? null,
      source: "binding",
    };
  }
...
export const resolveMediaFlowVariables = (
  ...
  const resolvedVariables = flow.variables.map((variable) => {
    ...
    if (
      ...
      return {
        ...
        ...resolved,
        value: null,
      } satisfies MediaResolvedFlowVariable;
    }
    return { ...variable, ...resolved } satisfies MediaResolvedFlowVariable;
  });
  const values = new Map(
    resolvedVariables.map((variable) => [variable.id, variable.value]),
  );
  const issueIdentities = new Set(
    issues.map(
      (issue) =>
        `${issue.code}\u001f${issue.variableId ?? ""}\u001f${issue.nodeId ?? ""}`,
    ),
  );
  ...
  const resolveValue = (value: unknown, nodeId: string): unknown => {
    ...
    if (Array.isArray(value))
      return value.map((entry) => resolveValue(entry, nodeId));
    if (value !== null && typeof value === "object") {
      return Object.fromEntries(
        Object.entries(value).map(([key, entry]) => [
          key,
          resolveValue(entry, nodeId),
        ]),
      );
    }
    return value;
...
export const replaceMediaFlowVariable = ({
  ...
  const nextFlow = {
    ...flow,
    updatedAt,
    variables: flow.variables.map((entry) =>
...
export const createMediaFlowPreset = ({
  ...
  const unresolved = resolveMediaFlowVariables(flow).variables.find(
    (variable) =>
      variable.value === null &&
      flow.variables.find((declaration) => declaration.id === variable.id)
        ?.required,
  );
  if (unresolved) {
...
export const updateMediaFlowPreset = ({
  ...
  const unresolved = resolution.variables.find(
    (variable) =>
      variable.value === null &&
      flow.variables.find((declaration) => declaration.id === variable.id)
        ?.required,
  ...
  if (unresolved)
    throw new Error(`Resolve ${unresolved.name} before updating a preset.`);
  const values = Object.fromEntries(
    resolution.variables.flatMap((variable) =>
      variable.value === null ? [] : [[variable.id, variable.value] as const],
    ),
  );
  return {
    ...flow,
...
export const removeMediaFlowPreset = ({
  ...
  return {
    ...
    updatedAt,
    activePresetId:
      flow.activePresetId === presetId ? null : flow.activePresetId,
