...
import type { MediaAssetPage, MediaAssetRecord } from "./contracts.js";
import {
...
describe("collectMediaAssetPages", () => {
  ...
  it("does not reuse a same-number revision from a different database", async () => {
    ...
    const loadPage = vi.fn(async () => page("database-b:1", 0, 1, [asset(1)]));
...
describe("matchesMediaAssetQuery", () => {
  ...
  it("adds human-friendly aliases for alpha workflow outputs", () => {
    ...
    expect(
      matchesMediaAssetQuery(candidate, "transparent cutout derived"),
    ).toBe(true);
  });
});
