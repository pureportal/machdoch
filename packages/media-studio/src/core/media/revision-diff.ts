...
const compareNodes = (
  ...
  const targetById = new Map(target.flow.nodes.map((node) => [node.id, node]));
  const nodeIds = [
    ...new Set([...baseById.keys(), ...targetById.keys()]),
  ].sort();
...
const compareEdges = (
  ...
  const targetById = new Map(target.flow.edges.map((edge) => [edge.id, edge]));
  const edgeIds = [
    ...new Set([...baseById.keys(), ...targetById.keys()]),
  ].sort();
  ...
  for (const edgeId of edgeIds) {
    ...
    if (before && !after) {
      changes.push({
        ...
        description: edgeIdentity(before),
      });
      continue;
    }
...
const compareVariables = (
  ...
  const beforeById = new Map(
    base.flow.variables.map((variable) => [variable.id, variable]),
  );
  const afterById = new Map(
    target.flow.variables.map((variable) => [variable.id, variable]),
  );
  ...
  for (const variableId of [
    ...
    const beforeHasBinding = Object.hasOwn(
      base.flow.variableBindings,
      variableId,
    );
    const afterHasBinding = Object.hasOwn(
      target.flow.variableBindings,
      variableId,
    );
    if (
      ...
      (beforeHasBinding &&
        !canonicalEqual(
          base.flow.variableBindings[variableId],
          target.flow.variableBindings[variableId],
        ))
    ) {
...
const comparePresets = (
  ...
  const beforeById = new Map(
    base.flow.presets.map((preset) => [preset.id, preset]),
  );
  const afterById = new Map(
    target.flow.presets.map((preset) => [preset.id, preset]),
  );
  ...
  for (const presetId of [
    ...
    if (!before || !after) continue;
    const changedFields: string[] = (
      ["name", "description", "values"] as const
    ).filter((field) => !canonicalEqual(before[field], after[field]));
...
export const createMediaFlowRevisionDiff = (
  ...
  if (base.flowId !== target.flowId) {
    throw new Error(
      "Flow revisions can only be compared within one flow identity.",
    );
  }
